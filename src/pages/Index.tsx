...
import { ArrowRight, LogOut, Loader2, ExternalLink, X } from 'lucide-react';
...
export default function Index() {
  ...
  return (
    <div className="min-h-screen bg-background grid-bg">
        ...
        <div className="hidden sm:flex max-w-5xl mx-auto px-4 h-14 items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {isLoggedIn && (
              <>
                <WalletMultiButton />
                <button onClick={signOut} className="flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-border text-muted-foreground text-[10px] font-semibold hover:text-foreground hover:border-border/80 transition-colors" title="Sign out">
                  <LogOut size={12} />
                  <span>Sign Out</span>
                </button>
              </>
        ...
        <div className="sm:hidden">
          ...
          <div className="flex items-center justify-between px-3 h-12">
            ...
            <div className="flex items-center gap-2">
              ...
              {isLoggedIn && <NotificationInline />}
              {isLoggedIn && (
                <button onClick={signOut} className="p-1.5 rounded-md text-muted-foreground hover:text-foreground transition-colors" title="Sign out">
                  <LogOut size={14} />
                </button>
              )}
            </div>
          </div>
          {/* Row 2: Wallet button full-width on mobile when logged in */}
          {isLoggedIn && (
      ...
      {/* Landing hero with email auth -- only when not logged in */}
      {!isLoggedIn && <LandingHero />}

      {/* Brand bar + tab nav -- visible when logged in */}
      {isLoggedIn && (
        <>
      ...
      {/* Password reset handler (modal overlay when reset link clicked) */}
      <PasswordResetHandler />

      {/* Cerberus AI Agent -- floating chat */}
      {isLoggedIn && <CerberusChat />}
